...
class TestCourse(APITestCase):
	def setUp(self):
		...
		self.test_course_kg = Course.objects.create(
			...
			lang_course='Kg')
		self.test_video = Video.objects.create(course=self.test_course, 
			name='test_name', 
	...
	def test_course_list(self):
		...
		self.assertTrue({'id':1, 'name':'Test Course','description':'Test description', 'lang_course':'Ru'}) in response.json()

	def test_course_detail(self):
		response = self.client.get(reverse('course_detail', kwargs={'pk':self.test_course.id}))
